...
def analyze_article(article_text: str) -> dict:
    ...
    # textract.parsers
    # article_text = textract.process('').decode('utf-8')
    ...
    for entity, entity_type in set(named_entities):
        ...
        if entity_sentences:
            sentiment = sum(sia.polarity_scores(
                sent)['compound'] for sent in entity_sentences) / len(entity_sentences)
        else:
            sentiment = 0  # or any other default value
        sentiment_data.append((entity, sentiment, entity_type))

    # Sort the scores from highest to lowest
    sorted_sentiment_data = sorted(
        sentiment_data, key=lambda x: x[1], reverse=True)

    # Function to merge single-named entities with the same sentiment score

    def merge_single_named_entities(sentiment_data):
        merged_data = []
        single_named_entities = {entity: (score, entity_type) for entity,
                                 score, entity_type in sentiment_data if len(entity.split()) == 1}
        ...
        merged_data.extend([(entity, score, entity_type) for entity, score,
                            entity_type in sentiment_data if len(entity.split()) > 1])
    ...
    df = pd.DataFrame(merged_sentiment_data, columns=[
        'Entity', 'Sentiment', 'IsPerson']).sort_values('Sentiment', ascending=False)
    df = df[df.IsPerson == 1]
